...
const Index = () => {
  ...
  >([]);
  const [calibrationClickPositions, setCalibrationClickPositions] = useState<
    { x: number; y: number }[]
  >([]);
              ...
              <div className="flex gap-4 items-center">
                <Button
                  ...
                  className={
                    calibrationMode
                      ...
                      : "bg-gray-100 hover:bg-gray-300 text-gray-800 border border-gray-300 manrope-light"
                  }
                >
                ...
                {calibration && (
                  <span className="text-sm text-slate-700">
                    Calibration: Mean Clicked LAB L*
                    {calibration.meanClickedLab.L.toFixed(1)}, a*
                    {calibration.meanClickedLab.a.toFixed(1)}, b*
                    {calibration.meanClickedLab.b.toFixed(1)} → Official Vita{" "}
                    {calibration.shade} LAB L*
                    {calibration.officialLab.L.toFixed(1)}, a*
                    {calibration.officialLab.a.toFixed(1)}, b*
                    {calibration.officialLab.b.toFixed(1)}
                  </span>
                )}
              </div>
                ...
                <div className="mt-4 w-full grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <Card className="w-full bg-gray-100 border border-gray-300">
                    <CardHeader className="pb-2 flex flex-row items-center justify-between font-manrope text-gray-900">
                      <CardTitle className="text-sm font-semibold font-manrope text-gray-900">
                        Calibration Image
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                          ...
                          <ColorAnalysisCanvas
                            ...
                            onCalibrationClick={async (x, y) => {
                              if (calibrationPoints.length < 5) {
                                setCalibrationClickPositions((prev) => [
                                  ...prev,
                                  { x, y },
                                ]);
                  ...
                  <div className="flex flex-col justify-start font-manrope text-gray-900">
                    ...
                    <div className="mb-4">
                      <div className="flex gap-2 items-center mb-3">
                        ...
                        <span className="text-xs ml-2 font-manrope text-gray-900">
                          Clicks: {calibrationPoints.length}/5
                        </span>
                      </div>
                      <Button
                        size="sm"
                      ...
                      {calibrationPoints.length < 3 && (
                        <span className="text-xs mt-2 block font-manrope text-gray-900">
                          Click at least 3 different areas of the Vita shade in
                      ...
                      {calibrationPoints.length >= 3 &&
                        calibrationPoints.length <= 5 && (
                          <div className="mt-2">
                            <span className="text-xs block mb-2 font-manrope text-gray-900">
                              You may click up to 5 points. Click 'Finish
                            ...
                            <Button
                              ...
                              title={
                                !calibrationShade
                                  ? "Select a Vita shade to enable calibration"
                                  : ""
                              }
                              onClick={async () => {
                                ...
                                const meanLab = {
                                  L:
                                    points.reduce((sum, p) => sum + p.L, 0) /
                                    points.length,
                                  a:
                                    points.reduce((sum, p) => sum + p.a, 0) /
                                    points.length,
                                  b:
                                    points.reduce((sum, p) => sum + p.b, 0) /
                                    points.length,
                                };
                                ...
                                try {
                                  ...
                                  const official = shades.find(
                                    (s) => s.name === calibrationShade
                                  );
                                  if (official) {
                                    setCalibration({
                                  ...
                                  toast({
                                    ...
                                    description:
                                      "Could not load shade data. Please try again.",
                                    variant: "destructive",
                                    duration: 3000,
                                  });
                                }
            ...
            <Card className="modern-shadow-lg hover-lift border-0 bg-[#ecedef] backdrop-blur-sm border border-gray-400">
              <CardHeader className="pb-6">
                ...
                <p className="text-slate-600 text-sm">
                  Click on the tooth to analyze color values and get shade
                  recommendations
                </p>
              </CardHeader>
              <CardContent>
                    ...
                    >
                      <ColorAnalysisCanvas
                        ...
                        calibrationLab={
                          calibration
                            ? {
                                clickedLab: calibration.meanClickedLab,
                                officialLab: calibration.officialLab,
                              }
                            : undefined
                        }
                  ...
                  <div className="flex items-center justify-center h-96 rounded-2xl border-2 border-gray-400 bg-[#ecedef]/80">
                    <div className="text-center">
                      <p className="text-slate-600 text-lg">
                        Please upload an image in the panel above to begin
                        analysis
                      </p>
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            ...
            <Card className="modern-shadow hover-lift border-0 bg-[#ecedef] backdrop-blur-sm border border-gray-400">
              <CardHeader className="pb-4">
                <CardTitle className="text-xl text-gray-800 manrope-light">
                  Reference Shade
                </CardTitle>
                <p className="text-sm text-slate-600 manrope-extralight">
                  Compare with known shade references
                </p>
              </CardHeader>
              <CardContent className="space-y-4">
                <div>
                  <Label
                    htmlFor="reference-shade"
                    className="text-sm font-medium text-slate-700"
                  >
                    Known Reference Shade
                  </Label>
                  <Select
                    value={referenceShade}
                    onValueChange={setReferenceShade}
                  >
                    ...
                    <SelectContent className="bg-[#ecedef] border border-gray-400 shadow-lg z-50">
                      <SelectItem
                        value="A1"
                        className="text-black hover:bg-[#e5e7ea] focus:bg-[#e5e7ea]"
                      >
                        A1
                      </SelectItem>
                      <SelectItem
            ...
            <Card className="modern-shadow hover-lift border-0 bg-[#ecedef] backdrop-blur-sm border border-gray-400">
              ...
              <CardContent className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="p-4 bg-[#ecedef] rounded-xl border border-gray-400">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm text-slate-600 manrope-extralight">
                          Closest Vita match:
                        </span>
                        <Badge
                          variant="secondary"
                          className="bg-gray-200 text-gray-800 hover:bg-gray-200 border border-gray-400"
                        >
                          {typeof selectedPixelData.vitaShade === "object"
                            ? selectedPixelData.vitaShade?.name ||
                              JSON.stringify(selectedPixelData.vitaShade)
                            : selectedPixelData.vitaShade}
                        </Badge>
                      </div>
                      <div className="text-xs text-slate-500">
                        <div>
                          Pixel LAB: L*{selectedPixelData.lab?.L?.toFixed(1)},
                          a*{selectedPixelData.lab?.a?.toFixed(1)}, b*
                          {selectedPixelData.lab?.b?.toFixed(1)}
                        </div>
                        {selectedPixelData.adjustedLab && (
                          <div>
                            Adjusted LAB: L*
                            {selectedPixelData.adjustedLab.L.toFixed(1)}, a*
                            {selectedPixelData.adjustedLab.a.toFixed(1)}, b*
                            {selectedPixelData.adjustedLab.b.toFixed(1)}
                          </div>
                        )}
                        <div>
                          Vita LAB:{" "}
                          {selectedPixelData.vitaLab
                            ? `L*${selectedPixelData.vitaLab.L?.toFixed(1)}, a*${selectedPixelData.vitaLab.a?.toFixed(1)}, b*${selectedPixelData.vitaLab.b?.toFixed(1)}`
                            : "N/A"}
                        </div>
                ...
                <div className="space-y-3">
                  <Button
                    className="w-full bg-[#e5e7ea] hover:bg-gray-300 text-gray-800 rounded-xl py-3 shadow-lg"
                    disabled
                  >
              ...
              <div className="flex gap-3">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    toast({
                      title: "Case saved locally!",
                      description: `Case '${caseName}' has been saved.`,
                    });
                  }}
                >
